...
class KP184Splitter extends IPSModule
{
	...
	{	 
		...
		if($this->debugLevel >= 6) { $this->AddDebugLogEntry(__FUNCTION__ . "-BufferIN", $bufferIN, 1); }
		
		if($this->startsWith($bufferIN,"\x01\x03")) {
			$len = strlen($bufferIN);
			if($len < 23) {
				SetValueString($bufferID, $bufferIN);
			} else {
				SetValueString($bufferID, '');			
				if($this->debugLevel >= 4) { $this->AddDebugLogEntry(__FUNCTION__ . ">SendDataToChildren", $bufferIN, 1); }
				
				// Weiterleitung zu allen GerÃ¤t-/Device-Instanzen
				$this->SendDataToChildren(json_encode(Array("DataID" => "{CDE25829-ED60-22E3-B8A7-DD02C7B1340F}", "Buffer" => $bufferIN))); // Splitter Interface GUI
			}
		} else {
			if($this->debugLevel >= 3) { $this->AddDebugLogEntry(__FUNCTION__, "Warning - Buffer CLEAR", 0); }
			SetValueString($bufferID, '');
		}
	}
